...
class DateTimeParser:
    ...
    def numberOfDaysForGivenYears(self, years):
        # Get the current year
        self.currentYear = datetime.now(timezone.utc).year

        # Calculate the target year after adding the specified number of years
        targetYear = self.currentYear + years

        # Initialize the number of days to add
        totalDays = years * 365

        # Adjust for leap years
        for year in range(self.currentYear, targetYear):
    ...
    def parse_input(input_time):
        try:
            ...
            if input_time.lower() == "now":
                ...
                if input_time.lower().endswith("minutes") or input_time.lower().endswith(
                        "mins") or input_time.lower().endswith("minute") or input_time.lower().endswith("min"):
                    time = datetime.now(timezone.utc) + timedelta(minutes=integer)
                if input_time.lower().endswith("hours") or input_time.lower().endswith(
                        "hrs") or input_time.lower().endswith("hour") or input_time.lower().endswith("hr"):
                ...
                if input_time.lower().endswith("years") or input_time.lower().endswith(
                        "yrs") or input_time.lower().endswith("year") or input_time.lower().endswith("yr"):
                    time = datetime.now(timezone.utc) + timedelta(
                        days=DateTimeParser.numberOfDaysForGivenYears(integer))
                ...
                for f in formats:
                    try:
                        return datetime.strptime(input_time, f).astimezone(timezone.utc)
                    except ValueError:
                        continue
